...
import { getExpenses, updateExpenseBill, updateExpenseType, updateExpenseApplication, updateExpenseCash, createExpense, checkDuplicates, bulkImportExpenses, deleteExpense } from '../api/client';
  ...
  return useMutation({
    ...
    onMutate: async ({ id, isCash }) => {
      ...
      queryClient.setQueryData<Expense[]>([EXPENSES_KEY, groupId], (old) => {
        if (!old) return old;
        return old.map((expense) =>
          expense.id === id ? { ...expense, Bargeldabhebung: isCash } : expense
        );
      });
...
export const useCheckDuplicates = () => {
  return useMutation({
    mutationFn: ({ 
      ...
      groupId: number;
    }) => checkDuplicates(expenses, groupId),
  });
};
...
export const useBulkImportExpenses = () => {
  ...
  return useMutation({
    ...
    }) => bulkImportExpenses(data, data.groupId),
    onSuccess: () => {
